...
Эти шаги относятся к скачиваемому рецепту с именованным Docker volume.
Если вы использовали генератор конфигурации, следуйте `README.md` из его
комплекта: в нём уже есть `.env`, используется выбранный каталог данных,
а команда восстановления требует `-y`.
...
Откройте `http://localhost:3001`. На экране выбора базы данных выберите **SQLite**
и нажмите **Далее**. Этот рецепт использует SQLite: не выбирайте встроенную
MariaDB или внешнюю базу для этой конфигурации. Затем создайте администратора
и сохраните пароль в менеджере паролей.

Нажмите **Добавить монитор**, оставьте тип **HTTP(s)**, укажите понятное название
и полный URL своего сервиса, затем сохраните проверку. Интервал по умолчанию —
60 секунд. Дождитесь успешной проверки и посмотрите время ответа, прежде чем
добавлять остальные мониторы. HTTP-проверка подтверждает доступность адреса,
а не работу всех функций сервиса.

Чтобы поделиться доступностью, откройте раздел страниц статуса, создайте страницу
с названием и слагом, добавьте нужные мониторы и сохраните. Откройте полученный
адрес `/status/<slug>` в приватном окне браузера, чтобы проверить публичный вид.
...
После перехода контейнера в healthy войдите заново и проверьте названия и адреса
мониторов, историю проверок и публичную страницу статуса. Работающий контейнер
сам по себе не подтверждает, что нужные данные восстановлены.
