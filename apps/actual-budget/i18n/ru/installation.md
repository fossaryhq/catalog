...
volume, распаковывает архив и поднимает сервер. После этого каждый клиент
окажется «новее» восстановленного сервера: откройте каждый браузер и настольное
приложение, а если клиент отказывается синхронизироваться — удалите его
локальный файл и заново скачайте бюджет с сервера. Практическую проверку
восстановления процедура не проходила: отрепетируйте её на отдельном сервере.
