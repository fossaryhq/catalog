...
### Архивный номер занят, хотя такого документа нет

```
Document with this Archive Serial Number already exists in the trash.
```

Удалённый документ попадает в корзину и сохраняет там свой архивный номер на
весь срок хранения, поэтому номер остаётся занятым, пока самого документа нет ни
в одном списке. Откройте **Корзину** в боковом меню, удалите документ
окончательно и присвойте номер заново. То же происходит с документом, который
поглощается повторно после удаления первой копии.
